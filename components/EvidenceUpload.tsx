...
export const EvidenceUpload: React.FC<EvidenceUploadProps> = ({
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <h2 className="text-2xl font-bold text-white mb-2 text-center">Evidence Locker</h2>
      ...
      <div className="bg-dark-700 p-6 rounded-xl border border-dark-600">
        <div className="grid grid-cols-1 gap-4 mb-6">
          <label className="relative block cursor-pointer group">
            <input
              ...
              ref={fileInputRef}
            />
            <div className="h-32 border-2 border-dashed border-dark-500 rounded-xl hover:border-violet-500/50 hover:bg-violet-500/10 transition-all duration-200 flex flex-col items-center justify-center text-center">
              <div className="w-10 h-10 bg-violet-500/20 rounded-full flex items-center justify-center mb-2 group-hover:scale-110 transition-transform duration-200">
        ...
        {fileTypeError && (
          <div className="mb-6 bg-red-500/10 border border-red-500/30 rounded-lg p-4 flex items-start gap-3 animate-fade-in">
            <AlertCircle className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
            <div>
              <p className="font-medium text-red-300">Invalid File Type</p>
              <p className="text-sm text-red-400 mt-1">{fileTypeError}</p>
        ...
        {files.length > 0 && (
          <div className="space-y-3 mb-6">
            ...
            {files.map((f, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-dark-600 rounded-lg border border-dark-500">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-200 truncate max-w-[200px]">{f.name}</p>
                    <div className="flex items-center gap-2">
                        <p className="text-xs text-slate-500 uppercase">{f.type.split('/')[1]}</p>
                        {f.classification && (
                            <span className="flex items-center gap-1 text-[10px] bg-violet-500/20 text-violet-300 px-1.5 py-0.5 rounded font-bold border border-violet-500/30">
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-500 hover:to-violet-400 disabled:from-dark-600 disabled:to-dark-600 text-white py-3 rounded-xl font-medium shadow-glow flex items-center justify-center gap-2 transition-all duration-200"
        >
          {isProcessing ? (
            <>
